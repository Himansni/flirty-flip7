-- Forward-only repair: qualify reconnect predicates that collide with output-column names.
...
begin
  ...
  if found and v_room.status not in ('closed','expired') and v_room.expires_at > now() then
    update public.couple_game_participants as p
       set nickname = v_nickname, last_seen_at = now()
     where p.room_id = v_room.id and p.player_number = v_existing and p.left_at is null;
