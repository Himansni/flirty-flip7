...
test("browser adapter stays isolated and stores only scoped reconnect state in sessionStorage", () => {
  ...
  assert.match(onlineSource, /realtime\.setAuth\(runtime\.session\.access_token\)/);
  ...
  assert.match(onlineSource, /runtime\.room\.status === "active" && \(previousStatus !== "active" \|\| previousGameId !== runtime\.room\.game_id\)/);
});
